...
testRatio = 0.2  # if there are 1000 images then split will be 200 for testing
validationRatio = 0.2  # if there are 1000 images then 20% of remaining 800 will be 160 for validation
###################################################
...
for x in range(0, len(myList)):
    ...
    count += 1
print(" ")
# Converting the images and classes to arrays
...
# To check if number of images matches for each label in dataset
print("Data Shapes")
...
# Display a bar chart showing no. of samples
...
def preprocessing(img):
    ...
    img = img / 255  # To normalize values from 0 to 255 to 0 to 1
    return img
...
dataGen = ImageDataGenerator(width_shift_range=0.1,
                             ...
                             zoom_range=0.2,  # It can go from 0.8 to 1.2
                             shear_range=0.1,  # Magnitude of shear angle
                             rotation_range=10)  # Degrees
dataGen.fit(X_train)
batches = dataGen.flow(X_train, y_train,batch_size=20)
# Requesting data generator to generate images, Batch size = No. of images craeted each time it's called
...
def myModel():
    no_Of_Filters = 60
    size_of_Filter = (5, 5)  # This is the kernel that moves around the image to get features
    # This would remove 2 Pixels from each border When using 32*32 image
    size_of_Filter2 = (3, 3)
    size_of_pool = (2, 2)  # Scale down all feature map to generalize more, and reduce over fitting
    no_Of_Nodes = 500  # No. of nodes in hidden layer
    model = Sequential()
    model.add((Conv2D(no_Of_Filters, size_of_Filter, input_shape=(imageDimensions[0], imageDimensions[1], 1),activation='relu')))
    # Adding more convolutional layers = fewer features but can cause accuracy to increase
    ...
    model.add(Dropout(0.5))  # Input node to drop in each update: 1-ALL 0-NONE
